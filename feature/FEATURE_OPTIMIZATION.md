...
所有模块的 `build/` 目录均已排除。文档中的“当前问题”和“修改方案”保留实施前的审计证据，实际状态以各节的“实施状态”和“实施结果”为准。

截至 2026-08-31，本轮 Feature 优化已结项：

| 范围 | 状态 | 说明 |
| --- | --- | --- |
| 认证 3.1～3.3 | 已完成 | 密码存储、验证码时序和 QQ 登录订阅问题均已处理。 |
| 认证 3.4 | 暂缓 | 后端尚未提供未登录找回密码接口，不使用需要登录态的更新密码接口替代。 |
| 订单、商品和反馈 4.1～4.4 | 已完成 | 重复提交、购物车清理、支付确认和可空字段处理均已完成。 |
| 客服、WebView 和通用状态 5.1～5.4 | 已完成 | WebSocket、WebView、生命周期收集和地址表单校验均已完成。 |
| 低优先级整理项 | 已完成 | 已处理确认需要修改的条目；`Modifier` 顺序采用渐进统一，Jetifier 保留为已接受兼容配置。 |
...
- 联系人、省、市、区和详细地址均按去除首尾空白后的非空值校验，手机号复用项目统一的大陆手机号规则。
- ViewModel 通过只读 `StateFlow` 暴露表单有效性和保存中状态；页面仅在详情加载成功且表单有效、没有保存请求时启用保存按钮。
- `saveAddress()` 使用同一规则重新校验当前字段快照，并规范化首尾空白后再构造请求，不能通过绕过页面按钮提交无效地址。
- 保存请求启动前同步取得提交锁；新增和编辑请求在成功、失败或取消后统一恢复状态，连续点击不会产生重复请求。
- 保存中使用底部按钮现有加载效果，成功后继续通过地址域导航结果返回并刷新列表，失败则保留表单供用户重试。
- 地址详情 Route 的状态收集同步改为生命周期感知方式；Repository、接口、模型、导航契约和中英文资源均未改变。
- 新增 ViewModel 单元测试覆盖字段校验、绕过 UI、新增参数规范化、编辑回填、重复提交、失败恢复重试和成功返回刷新结果。
...
各独立批次已根据对应风险增加或更新测试。后续修改相关逻辑时，至少继续覆盖以下回归场景：
...
- 合法表单只提交一次，提交期间显示保存状态。
- 新增和编辑成功后返回刷新，失败后恢复保存状态并允许重试。
...
本轮已按照以下顺序拆分为独立批次实施；找回密码因缺少后端接口未纳入实现：
...
各批均保持独立修改、独立测试和独立提交，避免问题之间相互牵连并保留单独回滚能力。

## 10. 剩余事项与结项结论

本轮审计确认的高优先级和通用优化项均已完成，当前没有需要继续实施的 Feature 优化批次。以下事项不计为本轮未完成：

- 找回密码继续暂缓，等待后端提供未登录验证码和密码重置接口及明确请求契约。
- 服务端如后续支持订单幂等键，应再为创建订单增加服务端最终幂等保障。
- 商品分享、客服相册和拍摄属于尚未定义完整交互与接口契约的产品功能，应在需求确认后独立实施；近期不提供时应评估隐藏无效入口。
- Compose `Modifier` 参数顺序在后续修改相关组件时渐进统一，不进行无行为收益的全仓机械改写。
- 支付、QQ 登录、WebView、系统导航栏和页面交互仍属于每次发布前的设备回归范围，不以 JVM 单元测试或编译结果替代。

后续新增 Feature 需求或发现新问题时，应重新审计并建立独立批次，不继续向本轮结项范围追加无关修改。
